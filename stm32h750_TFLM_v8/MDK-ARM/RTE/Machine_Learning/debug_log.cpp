...
#include "uart_use.h"  // Содержит Debug_LogString и все необходимые прототипы

/* ============== УДАЛЕНО: все определения DMA и UART ============== */
// Вся работа с UART и DMA теперь в uart_use.cpp
// НЕТ: usart.h, dma_busy, debug_dma_send, и т.д.
...
void InvokeDebugLogCallback(const char* s) {
  if (debug_log_callback != nullptr) {
    ...
    /* Debug_LogString определена в uart_use.cpp */
    Debug_LogString(s);
  }
}
...
/* ============== УДАЛЕНО: ДОПОЛНИТЕЛЬНЫЕ ФУНКЦИИ ============== */
// DebugLog_Printf, DebugLog_IsReady, DebugLog_WaitReady - удалены
// Используйте функции из uart_use.h вместо них:
// - Debug_Printf() вместо DebugLog_Printf()
// - Debug_IsReady() вместо DebugLog_IsReady()
// - Debug_Flush() вместо DebugLog_WaitReady()
